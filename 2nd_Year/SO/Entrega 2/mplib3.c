...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
  ...
  // Criacao dos canais e inicializacao de mutexes e variaveis de condicao
  for (i=0; i<number_of_tasks; i++) {
    for (j=0; j<number_of_tasks; j++) {
      ...
      if(pthread_mutex_init(&(channel_array[i*number_of_tasks+j]->mutex), NULL) != 0) {
        fprintf(stderr, "\nErro ao inicializar mutex\n");
        return -1;
      }

      if(pthread_cond_init(&(channel_array[i*number_of_tasks+j]->wait_for_free_space), NULL) != 0) {
        fprintf(stderr, "\nErro ao inicializar variável de condição\n");
        return -1;
      }

      if(pthread_cond_init(&(channel_array[i*number_of_tasks+j]->wait_for_messages), NULL) != 0) {
        fprintf(stderr, "\nErro ao inicializar variável de condição\n");
        return -1;
      }
    }
...
void libertarMPlib() {
  ...
  // Destruicao dos mutexes e variaveis de condicao
  for (i=0; i<number_of_tasks; i++) {
    for (j=0; j<number_of_tasks; j++) {

      if(pthread_mutex_destroy(&(channel_array[i*number_of_tasks+j]->mutex)) != 0) {
        fprintf(stderr, "\nErro ao destruir mutex\n");
        exit(EXIT_FAILURE);
      }

      if(pthread_cond_destroy(&(channel_array[i*number_of_tasks+j]->wait_for_free_space)) != 0) {
        fprintf(stderr, "\nErro ao destruir variável de condição\n");
        exit(EXIT_FAILURE);
      }

      if(pthread_cond_destroy(&(channel_array[i*number_of_tasks+j]->wait_for_messages)) != 0) {
        fprintf(stderr, "\nErro ao destruir variável de condição\n");
        exit(EXIT_FAILURE);
      }
